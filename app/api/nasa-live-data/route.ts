/**
 * NASA Real-Time Data API Routes
 * Serves live NASA satellite data to the 3D farming game
 */
...
const dataFetcher = new RealTimeNASADataFetcher(
  process.env.NEXT_PUBLIC_NASA_API_KEY || 'xUVqGzhFLydz87SVBPKEXlQnHo9VBwCfpij5AzCj'
)

// Main endpoint for getting all NASA data for a farm location
export async function GET(request: NextRequest) {
  try {
    const { searchParams } = new URL(request.url)
    const lat = parseFloat(searchParams.get('lat') || '42.0308')
    const lng = parseFloat(searchParams.get('lng') || '-93.5805')
    const radius = parseFloat(searchParams.get('radius') || '0.1') // degrees
    ...
    // Process data for game mechanics
    const gameData = processDataForGame(nasaData, lat, lng)

    // Add metadata for educational purposes
    const response = {
      success: true,

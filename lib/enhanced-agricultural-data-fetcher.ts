    ...
    try {
      ...
      const baseUrl = typeof window !== 'undefined' 
        ...
        : (process.env.VERCEL_URL 
          ...
          : 'http://localhost:3000')  // Use port 3000 where server is actually running
      
      const fullUrl = `${baseUrl}${this.nasaEndpoint}?lat=${latitude}&lng=${longitude}`
      
      console.log(`🌍 Fetching NASA data from: ${fullUrl}`)
      console.log(`🔧 Base URL: ${baseUrl}, Endpoint: ${this.nasaEndpoint}`)
